...
public class CannyEdgeDetector {
    ...
    private int mBaseImageWidth = 1024;
    ...
    public CannyEdgeDetector( CameraHandler sketchy ) {
        cameraHandler = sketchy;


        SharedPreferences prefs=PreferenceManager.getDefaultSharedPreferences(sketchy.getContext());

        String lowThresholdS = prefs.getString("lowThreshold", "2.5");  // real default is set in Settings
        String highThresholdS = prefs.getString("highThreshold", "5.0");
        ...
        gaussianKernelRadius = (Float.valueOf(gaussianKernelRadiusS) * cameraHandler.getWidth()) / mBaseImageWidth;
        gaussianKernelWidth = (Integer.valueOf(gaussianKernelWidthS) * cameraHandler.getWidth()) / mBaseImageWidth;
    ...
    public void setSourceImage(Bitmap image) {
        sourceImage = image;
        edgesImage = Bitmap.createBitmap(sourceImage.getWidth(),sourceImage.getHeight(), Bitmap.Config.ARGB_8888);
    }
    ...
    public void process() {
        ...
        Log.e(TAG,"CANNY READ LUMINANCE");
        readLuminance();

        Log.e(TAG,"Canny Normalise");

        if (contrastNormalized) normalizeContrast();
        Log.e(TAG,"Canny GRADIENTS");
        ...
        Log.e(TAG,"Canny hysteresis");

        performHysteresis(low, high);

        Log.e(TAG,"Canny threshold");

        thresholdEdges();
        Log.e(TAG,"Canny Write");
